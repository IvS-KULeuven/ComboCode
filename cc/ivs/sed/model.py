...
from cc.ivs.sigproc import interpol
...
caldir = os.path.join(cc.path.ivsdata,'sedtables','calibrators')
...
def set_defaults(*args,**kwargs):
    ...
    """
    clear_memoization(keys=['cc.ivs.sed.model'])
...
#                 #'marcs','marcs2','comarcs','tlusty','uvblue','atlas12']
        return ['marcs','marcs2','comarcs']
    else:
        #files = config.glob(basedir,'*%s*.fits'%(grid))
        fn_grids = os.path.join(cc.path.atm,'*{}*.fits'.format(grid))
        files = glob.glob(fn_grids)
        files.sort()
    ...
    if not '*' in basename:
        if use_scratch:
            ...
            if integrated:
                ...
                grid = os.path.join(cc.path.atm,'i'+basename)
            else:
                #grid = config.get_datafile(basedir,basename)
                grid = os.path.join(cc.path.atm,basename)
    #-- we could also ask for a list of files, when wildcards are given:
        ...
        if use_scratch:
            ...
            if integrated:
                ...
                fn_grid = os.path.join(cc.path.atm,'i'+basename)
                grid = glob.glob(fn_grid)
                grid.sort()
            else:
                #grid = config.glob(basedir,basename)   
                fn_grid = os.path.join(cc.path.atm,basename)
                grid = glob.glob(fn_grid)
                grid.sort()  
...
def list_calibrators(library='calspec'):
    ...
    fn_grid = os.path.join(caldir,library,'*.fits')
    files = glob.glob(fn_grid)
    files.sort()
...
def read_calibrator_info(library='ngsl'):
    ...
    filename = os.path.join(caldir,'{}.ident'.format(library))
    names = []
    fits_files = []
    phot_files = []
    with open(filename,'r') as ff:
        for line in ff.readlines():
            ...
            try:
                ...
                fits_file = os.path.join(caldir,line[1])
                phot_file = os.path.join(caldir,line[2])
