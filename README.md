...
First and foremost, you require a Python distribution installed on your machine. I recommend Anaconda, which allows for very flexible package management. If you have the Anaconda distribution installed, you only require the pyfits and ephem packages to be installed in addition to the Anaconda basic package.
...
Thirdly, you need the IvS repository, which can be installed as described <a href="https://github.com/robinlombaert/IvSPythonRepository"> here</a>. However, replace the github.com link with one that points to my version of the code, i.e. replace *JorisDeRidder* with *robinlombaert*. This also requires GFortran to be installed your machine, so you can compile the Fortran modules of the repo. With the IvS repository comes data that are used by the package. The bare minimum required to run ComboCode is available <a href="http://ster.kuleuven.be/~robinl/cc/ivsdata.tar.gz"> here</a>. Make sure to update the config.py file with the location of the unpacked ivsdata upon installation of the IvS repository. Make sure the IvS repository is included in the PYTHON\_PATH in your ~/.bash_profile. 
